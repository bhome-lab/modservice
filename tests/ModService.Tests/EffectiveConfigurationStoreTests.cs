...
public sealed class EffectiveConfigurationStoreTests
{
    ...
    {
        var initial = CreateValidConfiguration();
        ...
        Assert.Equal("repo", effectiveBeforeReload.Executor.Source);

        var invalid = CreateInvalidConfiguration();
        monitor.Set(invalid);

        Assert.True(store.TryGetCurrent(out var effectiveAfterReload));
        Assert.Equal("repo", effectiveAfterReload.Executor.Source);
    }
    ...
    public void AcceptsFirstValidReload_WhenStartupConfigurationIsInvalid()
    {
        ...
        Assert.Equal("repo", effective.Executor.Source);
    }
    ...
    public void Status_TracksValidationErrors_AndVersion()
    {
        ...
        monitor.Set(CreateInvalidConfiguration());
        ...
        monitor.Set(CreateValidConfiguration());
    ...
    private static ModServiceConfiguration CreateValidConfiguration()
    {
    ...
    private static ModServiceConfiguration CreateInvalidConfiguration()
    {
        var configuration = CreateValidConfiguration();
        configuration.Executor.Source = "missing";
