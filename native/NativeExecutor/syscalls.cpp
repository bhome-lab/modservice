...
extern "C" uint32_t g_syscall_ssn    = 0;
...
NTSTATUS syscall::NtCreateThreadEx(PHANDLE th, ACCESS_MASK access, PVOID oa,
    ...
    SYSCALL_PREPARE(SyscallId::NtCreateThreadEx);
    using fn = NTSTATUS(NTAPI*)(PHANDLE, ACCESS_MASK, PVOID, HANDLE,
                                 PVOID, PVOID, ULONG, SIZE_T, SIZE_T, SIZE_T, PVOID);
    return reinterpret_cast<fn>(&indirect_syscall_stub)(th, access, oa, ph,
                                                         start, arg, flags, zero_bits,
                                                         stack, max_stack, attr);
}

NTSTATUS syscall::NtWaitForSingleObject(HANDLE h, BOOLEAN alertable, PLARGE_INTEGER timeout) {
    SYSCALL_PREPARE(SyscallId::NtWaitForSingleObject);
    using fn = NTSTATUS(NTAPI*)(HANDLE, BOOLEAN, PLARGE_INTEGER);
    return reinterpret_cast<fn>(&indirect_syscall_stub)(h, alertable, timeout);
}

NTSTATUS syscall::NtQueryInformationProcess(HANDLE ph, ULONG cls, PVOID info,
                                             ULONG len, PULONG ret_len) {
    SYSCALL_PREPARE(SyscallId::NtQueryInformationProcess);
    using fn = NTSTATUS(NTAPI*)(HANDLE, ULONG, PVOID, ULONG, PULONG);
    return reinterpret_cast<fn>(&indirect_syscall_stub)(ph, cls, info, len, ret_len);
}

#undef SYSCALL_PREPARE
